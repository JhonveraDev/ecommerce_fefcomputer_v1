...
function Storefront() {
  ...
  const getPage = (hash = window.location.hash) => hash.startsWith('#tienda') ? 'store' : hash.startsWith('#producto/') ? 'product' : hash === '#carrito' ? 'cart' : hash === '#favoritos' ? 'wishlist' : 'home';
  ...
  const sharedHeader = <Header cartCount={itemCount} wishlistCount={wishlistCount} />;
  ...
  if (page === 'wishlist') return <>{sharedHeader}<WishlistPage onProductClick={openProduct} onAddToCart={addToCart} /><Footer {...footerData} /></>;
