...
export function AdminProductsPage() {
  ...
  const save = (product) => { setEditing(null); setProducts((current) => [product, ...current.filter((item) => item.id !== product.id)]); setOptions((current) => ({ categories: product.category && !current.categories.some((item) => item.name === product.category) ? [...current.categories, { id: product.categoryId, name: product.category }] : current.categories, brands: product.brand && !current.brands.some((item) => item.name === product.brand) ? [...current.brands, { id: product.brandId, name: product.brand }] : current.brands })); };
  const archive = async (product) => { if (!window.confirm(`¿Archivar “${product.name}”? Dejará de estar disponible para venta.`)) return; try { await request(`/admin/products/${product.id}`, { method: 'DELETE' }); setProducts((current) => includeArchived ? current.map((item) => item.id === product.id ? { ...item, status: 'ARCHIVED' } : item) : current.filter((item) => item.id !== product.id)); } catch (requestError) { setError(requestError.message); } };
