...
import { AlertTriangle, Boxes, CircleAlert, FilePenLine, Image as ImageIcon, Pencil, Plus, RotateCcw, Search, ShieldAlert, Trash2, X } from 'lucide-react';
...
function ProductForm({ product, options, onCancel, onSaved }) {
  ...
  const submit = async (event) => {
    ...
    setSaving(true);
    try {
      const payload = { ...values, basePrice: Number(values.basePrice), compareAtPrice: values.compareAtPrice === '' || values.compareAtPrice == null ? null : Number(values.compareAtPrice), physicalQuantity: Number(values.physicalQuantity), reorderPoint: Number(values.reorderPoint) };
...
export function AdminProductsPage() {
  ...
  return <main className={styles.page}>
    ...
    {editing && <ProductForm product={editing} options={options} onCancel={() => setEditing(null)} onSaved={save} />}
