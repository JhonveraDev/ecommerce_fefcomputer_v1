...
function CartItem({ item, onProductClick }: { item: any; onProductClick: (product: any) => void }) {
  ...
  return <article className={styles.item}>
    <button className={styles.product} type="button" onClick={() => onProductClick(product)}><span className={styles.image}><img src={product.image} alt={product.name} /></span><span><b>{product.name}</b>{product.selectedVariant && <small>{Object.values(product.selectedVariant).join(' · ')}</small>}{product.sku && <small>SKU: {product.sku}</small>}{product.brand && <small>{product.brand}</small>}</span></button>
    ...
    <QuantityControl value={quantity} max={product.stock} onChange={(value) => updateQuantity(product.id, value, product.variantId || null)} />
    <strong className={styles.lineTotal}>{money(product.price * quantity)}</strong>
    <button className={styles.remove} type="button" aria-label={`Eliminar ${product.name}`} onClick={() => removeItem(product.id, product.variantId || null)}><Trash2 size={18} /></button>
  </article>;
}
...
function CartSummary() {
  ...
  const shipping = 0;
  return <aside className={styles.summary}><h2>Resumen del pedido</h2><dl><div><dt>Subtotal</dt><dd>{money(subtotal)}</dd></div>{savings > 0 && <div><dt>Descuentos</dt><dd className={styles.savings}>−{money(savings)}</dd></div>}<div><dt>Envío</dt><dd>{shipping ? money(shipping) : 'Gratis'}</dd></div><div className={styles.total}><dt>Total</dt><dd>{money(subtotal + shipping)}</dd></div></dl><a className={styles.checkoutButton} href="/checkout"><ShoppingBag size={18} />Proceder al pago</a><a href="/tienda">Continuar comprando</a></aside>;
}
