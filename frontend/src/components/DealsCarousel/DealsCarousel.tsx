...
type Product = { id:string; slug:string; name:string; brand:string; price:number; previousPrice:number|null; image:string; stock:number };
type Props = { products:Product[]; bannerImage:string; bannerTitle:string; bannerCtaLabel:string; onProductClick?:(p:Product)=>void; onAddToCart?:(p:Product)=>void; onAddToWishlist?:(p:Product)=>void; onCompare?:(p:Product)=>void; onQuickView?:(p:Product)=>void };
const money=(value:number)=>new Intl.NumberFormat('es-CO',{style:'currency',currency:'COP',maximumFractionDigits:0}).format(value);

function Card({product,onProductClick,onAddToCart,onAddToWishlist,onCompare,onQuickView}:{product:Product;onProductClick?:Props['onProductClick'];onAddToCart?:Props['onAddToCart'];onAddToWishlist?:Props['onAddToWishlist'];onCompare?:Props['onCompare'];onQuickView?:Props['onQuickView']}) {
  ...
  return <article className={styles.card}>
    ...
    <button className={styles.product} type="button" onClick={()=>onProductClick?.(product)}>
      <img src={product.image} alt=""/><p>{product.brand}</p><h3>{product.name}</h3>
      <div className={styles.prices}><b>{money(product.price)}</b>{product.previousPrice?<del>{money(product.previousPrice)}</del>:null}</div>
    </button>
    <div className={styles.stock}><span><i style={{width:'72%'}}/></span><p>Disponibles: {product.stock}</p></div>
    <button className={styles.add} type="button" onClick={()=>onAddToCart?.(product)}><ShoppingCart size={17}/>Agregar al carrito</button>
  </article>;
}

export function DealsCarousel({products,bannerImage,bannerTitle,bannerCtaLabel,onProductClick,onAddToCart,onAddToWishlist,onCompare,onQuickView}:Props) {
  const [tab,setTab]=useState('featured'); const [start,setStart]=useState(0); const [paused,setPaused]=useState(false);
  const deals=useMemo(()=>products.filter(p=>p.previousPrice&&p.previousPrice>p.price).sort((a,b)=>tab==='new'?b.id.localeCompare(a.id):0),[products,tab]);
  ...
  return <section className={styles.section} aria-label="Descuentos">
    <header><h2>Descuentos</h2><nav aria-label="Ordenar ofertas">{[['featured','Destacados'],['new','Recién agregados']].map(([id,label])=><button key={id} className={tab===id?styles.active:''} onClick={()=>choose(id)}>{label}</button>)}</nav></header>
